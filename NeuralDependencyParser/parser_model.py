...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        # embedings [0 - feature ]
        # END YOUR CODE
        return torch.tensor([torch.index_select(self.embeddings, 0, w[i]).view(-1).detach().numpy()
...
if __name__ == "__main__":
    ...
    def check_embedding():
        ...
        assert np.all(selected.data.numpy() ==
                      0), "The result of embedding lookup: " + repr(selected) + " contains non-zero elements."

    def check_forward():
        inputs = torch.randint(0, 100, (4, 36), dtype=torch.long)
        out = model(inputs)
        expected_out_shape = (4, 3)
        assert out.shape == expected_out_shape, "The result shape of forward is: " + \
            repr(out.shape) + " which doesn't match expected " + \
