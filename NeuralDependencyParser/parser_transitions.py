...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if (transition == 'S'):
            if (self.buffer):
                to_add = self.buffer.pop(0)
                self.stack.append(to_add)
        elif (len(self.stack) < 2):
            # Incorrect Prediction, causes trouble
            pass
        elif (transition == 'LA'):
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses):
        ...
        for partial_parse, trans in zip(parses, transitions):
            partial_parse.parse_step(trans)
        for i, unf_parse in enumerate(unfinished_parses):
            if (len(unf_parse.stack) == 1 and len(unf_parse.buffer) == 0):
                unfinished_parses.pop(i)
    dependencies = [pars.dependencies for pars in partial_parses]
    return dependencies
